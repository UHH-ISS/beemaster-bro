...
function conninfo_to_alertinfo(input: Conn::Info): AlertInfo {
    return connid_to_alertinfo(input$id, input$ts);
		return AlertInfo($timestamp = input$ts, $source_ip = addr_to_uri(input$id$orig_h), $source_port = port_to_count(input$id$orig_p), $destination_ip = addr_to_uri(input$id$resp_h), $destination_port = port_to_count(input$id$resp_p), $protocol = input$proto);
}
